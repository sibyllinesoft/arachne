...
class IRBenchmarkGenerator {
  ...
  generateNestedControlFlow(depth: number, branchingFactor: number): IRStatement[] {
    ...
    const createNestedIf = (currentDepth: number): IRStatement => {
      if (currentDepth === 0) {
        return {
          type: 'ExpressionStatement',
          expression: {
      ...
      const consequentStatements: IRStatement[] = [];
      const alternateStatements: IRStatement[] = [];
  ...
  generateNestedLoops(loopCount: number, bodySize: number): IRStatement[] {
    ...
    const createNestedLoop = (remaining: number): IRStatement => {
...
class BenchmarkRunner {
  private generator = new IRBenchmarkGenerator();
   ...
   */
  async benchmarkCFG(): Promise<BenchmarkSuite> {
    ...
    for (const size of testSizes) {
      ...
      results.push(await this.runBenchmark(
        `CFG Linear (${size} statements)`,
        size,
        () => cfgBuilder.buildFromStatements(linearProgram)
      ));

      // Nested control flow
      const nestedProgram = this.generator.generateNestedControlFlow(
        Math.ceil(Math.log2(size)), 
        Math.min(4, Math.ceil(size / 50))
      );
      results.push(await this.runBenchmark(
        `CFG Nested (depth ${Math.ceil(Math.log2(size))})`,
        nestedProgram.length,
        () => cfgBuilder.buildFromStatements(nestedProgram)
      ));
    }
   ...
   */
  async benchmarkSSA(): Promise<BenchmarkSuite> {
    const results: BenchmarkResult[] = [];
    const cfgBuilder = new CFGBuilder();
    ...
    for (const size of testSizes) {
      ...
      const dataFlowProgram = this.generator.generateComplexDataFlow(
        Math.ceil(size / 10), 
        size
      );
      ...
      results.push(await this.runBenchmark(
        `SSA Construction (${size} assignments)`,
        size,
        () => {
      ...
      results.push(await this.runBenchmark(
        `SSA Destruction (${size} assignments)`,
        size,
        () => {
   ...
   */
  async benchmarkPasses(): Promise<BenchmarkSuite> {
    const results: BenchmarkResult[] = [];
    const cfgBuilder = new CFGBuilder();
    ...
    for (const size of testSizes) {
      const program = this.generator.generateComplexDataFlow(
        Math.ceil(size / 10),
        size
      );
      ...
      results.push(await this.runBenchmark(
        `Constant Propagation (${size} assignments)`,
        size,
        () => {
      ...
      results.push(await this.runBenchmark(
        `Dead Code Elimination (${size} assignments)`,
        size,
        () => {
      ...
      // Copy Propagation
      results.push(await this.runBenchmark(
        `Copy Propagation (${size} assignments)`,
        size,
        () => {
   ...
   */
  async benchmarkPrinter(): Promise<BenchmarkSuite> {
    const results: BenchmarkResult[] = [];
    const cfgBuilder = new CFGBuilder();
    ...
    for (const size of testSizes) {
      ...
      results.push(await this.runBenchmark(
        `IR Printer with SourceMap (${size} statements)`,
        size,
        () => {
          for (const node of cfg.nodes.values()) {
            for (const stmt of node.instructions) {
  ...
  generateReport(): string {
    ...
    const totalMemory = this.results.reduce((sum, suite) => 
      sum + suite.results.reduce((suiteSum, r) => suiteSum + r.memoryUsed, 0), 0
    );
    ...
    const largestCFG = cfgResults.reduce((largest, current) => 
      current.inputSize > largest.inputSize ? current : largest, cfgResults[0] || { inputSize: 0, duration: Infinity }
    );
    ...
    const largestSSA = ssaResults.reduce((largest, current) => 
      current.inputSize > largest.inputSize ? current : largest, ssaResults[0] || { inputSize: 0, duration: Infinity }
    );
