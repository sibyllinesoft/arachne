...
exports.WildSamplesRunner = void 0;
 ...
 */
class WildSamplesRunner {
    outputDir;
    framework;
    startTime = 0;
    peakMemoryUsage = 0;
    constructor(outputDir = './artifacts/wild_sample_reports') {
        this.outputDir = outputDir;
        this.framework = new framework_js_1.DifferentialTestFramework((0, path_1.join)(this.outputDir, 'golden'));
    }
    /**
    ...
    async loadConfiguration(configPath) {
        try {
            const configContent = await fs_1.promises.readFile((0, path_1.resolve)(configPath), 'utf-8');
    ...
    async loadSamplesMetadata(config) {
        const wildSamplesConfig = config.wildSampleSettings;
        try {
            const metadataPath = (0, path_1.resolve)(wildSamplesConfig.metadataFile);
     ...
     */
    async generateTestCases(config, samplesMetadata) {
        const testCases = [];
        const wildSamplesConfig = config.wildSampleSettings;
        ...
        for (const configTestCase of config.testCases || []) {
            if (configTestCase.tags?.includes('wild')) {
                ...
                const metadata = samplesMetadata.find(m => m.filename.includes(sampleName) || m.source_metadata.sample_id === sampleName);
                const wildTestCase = {
                    ...configTestCase,
        ...
        if (samplesMetadata.length > 0) {
            for (const [index, metadata] of samplesMetadata.entries()) {
                ...
                const testCase = {
                    ...
                    bytecodeFile: (0, path_1.join)(wildSamplesConfig.corpusDirectory, metadata.filename),
     ...
     * Validate corpus quality
     */
    async validateCorpusQuality(config, testCases) {
        ...
        for (const testCase of testCases) {
            try {
                await fs_1.promises.access((0, path_1.resolve)(testCase.bytecodeFile));
            }
            catch {
    ...
    async runTests(testCases) {
        ...
        for (const testCase of testCases) {
            ...
            try {
                ...
                // Load sample content
                const sampleContent = await fs_1.promises.readFile((0, path_1.resolve)(testCase.bytecodeFile), 'utf-8');
                // Run parsing
                ...
                if (parseResult.success) {
                    // Run lifting
                    const liftStart = perf_hooks_1.performance.now();
                    liftResult = await mockLifter.lift(sampleContent);
                    const liftEnd = perf_hooks_1.performance.now();
                    if (liftResult.success) {
                        ...
                        const validationEnd = perf_hooks_1.performance.now();
                    }
                }
                const testEnd = perf_hooks_1.performance.now();
     ...
     */
    async createSyntheticSample(testCase) {
...
(function() {
    ...
    console.log(_0x456());
        ...
        const sampleDir = (0, path_1.resolve)(testCase.bytecodeFile, '..');
        await fs_1.promises.mkdir(sampleDir, { recursive: true });
        // Write synthetic sample
        await fs_1.promises.writeFile((0, path_1.resolve)(testCase.bytecodeFile), syntheticCode, 'utf-8');
        console.log(`   📝 Created synthetic sample: ${testCase.bytecodeFile}`);
    }
    /**
     * Generate enhanced statistics
     */
    generateStatistics(testCases, testResults) {
        const executionTime = perf_hooks_1.performance.now() - this.startTime;
        // Technique breakdown
        ...
        for (const result of testResults.results) {
            ...
            if (result.success)
                techniqueBreakdown[technique].passed++;
     ...
     */
    async generateReports(statistics, testResults, testCases) {
        ...
        const jsonReport = {
            ...
            testResults: testResults.results.map(result => ({
                ...
                classification: result.testCase.classification,
                wildSampleMetadata: result.testCase.wildSampleMetadata,
                semanticEquivalence: result.semanticEquivalence,
            })),
            corpusAnalysis: {
                totalSamples: testCases.length,
                techniqueDistribution: Object.fromEntries(Object.entries(statistics.techniqueBreakdown).map(([k, v]) => [k, v.total])),
                sourceDistribution: Object.fromEntries(Object.entries(statistics.sourceBreakdown).map(([k, v]) => [k, v.total])),
                difficultyDistribution: Object.fromEntries(Object.entries(statistics.difficultyBreakdown).map(([k, v]) => [k, v.total])),
            }
        };
        const reportPath = (0, path_1.join)(this.outputDir, 'wild-samples-report.json');
        await fs_1.promises.writeFile(reportPath, JSON.stringify(jsonReport, null, 2));
        // HTML report
        const htmlReport = this.generateHtmlReport(jsonReport);
        const htmlPath = (0, path_1.join)(this.outputDir, 'wild-samples-report.html');
        await fs_1.promises.writeFile(htmlPath, htmlReport);
...
<body>
    ...
    <div class="breakdown">
        ...
        ${Object.entries(stats.techniqueBreakdown).map(([technique, data]) => `
    ...
    <table>
        ...
        ${jsonReport.testResults.map((result) => `
...
${JSON.stringify(jsonReport, null, 2)}
    ...
    // Helper methods
    determinePrimaryTechnique(metadata) {
    ...
    determineDifficulty(metadata) {
        ...
        if (confidence > 0.9 && score > 0.8)
            return 'very_hard';
        if (confidence > 0.7 && score > 0.6)
            return 'hard';
        if (confidence > 0.5 && score > 0.4)
            return 'medium';
        return 'easy';
    }
    estimateOptimizationGains(metadata) {
        const techniques = metadata.classification.techniques;
...
exports.WildSamplesRunner = WildSamplesRunner;
/**
 * CLI entry point
 */
async function main() {
