...
describe('IRNodeFactory', () => {
  ...
  describe('binaryExpression', () => {
    ...
    it('should support all binary operators', () => {
      ...
      const operators: Array<IRBinaryExpression['operator']> = [
        '+', '-', '*', '/', '%', '**',
        '==', '!=', '===', '!==',
        '<', '<=', '>', '>=',
        '<<', '>>', '>>>',
  ...
  describe('blockStatement', () => {
    ...
    it('should create block with statements', () => {
      const statements = [
        { type: 'ExpressionStatement', expression: IRNodeFactory.identifier('x') } as any
      ];
    ...
    it('should include phi nodes when provided', () => {
      const phiNode = IRNodeFactory.phiNode(
        IRNodeFactory.createVariableName('x'),
        new Map(),
        IRNodeFactory.createSSAVersion(1)
      );
...
describe('Type Guards', () => {
  ...
  describe('isPhiNode', () => {
    it('should return true for phi nodes', () => {
      const phi = IRNodeFactory.phiNode(
        IRNodeFactory.createVariableName('x'),
        new Map(),
        IRNodeFactory.createSSAVersion(1)
      );
  ...
  describe('isSSAIdentifier', () => {
    it('should return true for SSA identifier nodes', () => {
      const ssaId = IRNodeFactory.ssaIdentifier(
        IRNodeFactory.createVariableName('x'),
        IRNodeFactory.createSSAVersion(1),
        'x'
      );
...
describe('IRUtils', () => {
  describe('clone', () => {
    it('should create deep copy of node', () => {
      const original = IRNodeFactory.binaryExpression(
        '+',
